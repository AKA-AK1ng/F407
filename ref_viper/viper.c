...
static viper_bench_totals_t g_viper_bench;

void viper_bench_reset(void) {
  memset(&g_viper_bench, 0, sizeof(g_viper_bench));
}

void viper_bench_get(viper_bench_totals_t *out) {
  if (out) *out = g_viper_bench;
}
...
void viper_bench_add_mul(uint64_t delta) {
#if VIPER_BENCH_ENABLE
  g_viper_bench.mul += delta;
  g_viper_bench.mul_count++;
#else
  (void)delta;
#endif
}

void viper_bench_add_as(uint64_t delta) {
#if VIPER_BENCH_ENABLE
  g_viper_bench.as += delta;
  g_viper_bench.as_count++;
#else
  (void)delta;
#endif
}

void viper_bench_add_atr(uint64_t delta) {
#if VIPER_BENCH_ENABLE
  g_viper_bench.atr += delta;
  g_viper_bench.atr_count++;
#else
  (void)delta;
#endif
}

void viper_bench_add_btr(uint64_t delta) {
#if VIPER_BENCH_ENABLE
  g_viper_bench.btr += delta;
  g_viper_bench.btr_count++;
#else
  (void)delta;
#endif
}

void viper_bench_add_stu(uint64_t delta) {
#if VIPER_BENCH_ENABLE
  g_viper_bench.stu += delta;
  g_viper_bench.stu_count++;
#else
  (void)delta;
#endif
}
...
#if VIPER_BENCH_ENABLE
  uint64_t t0 = viper_bench_now_cycles();
#endif
  viper_matvec(b, A, s);
#if VIPER_BENCH_ENABLE
  viper_bench_add_as(viper_bench_now_cycles() - t0);
#endif
  for (size_t i = 0; i < VIPER_K; i++) {
...
void viper_pke_enc(unsigned char *ct, const unsigned char *pk, const unsigned char m[32], const unsigned char omega[64]) {
  ...
  viper_matTvec(u, A, r);
#if VIPER_BENCH_ENABLE
  viper_bench_add_atr(viper_bench_now_cycles() - t0);
#endif
  for (size_t i = 0; i < VIPER_K; i++) {
  ...
  viper_encode(acc, m);
#if VIPER_BENCH_ENABLE
  t0 = viper_bench_now_cycles();
#endif
  viper_dot(t, bhat, r);
#if VIPER_BENCH_ENABLE
  viper_bench_add_btr(viper_bench_now_cycles() - t0);
#endif
  poly_add(acc, t);
...
void viper_pke_dec(unsigned char m[32], const unsigned char *skpke, const unsigned char *ct) {
  ...
  viper_dot(t, s, u);
#if VIPER_BENCH_ENABLE
  viper_bench_add_stu(viper_bench_now_cycles() - t0);
#endif
  poly_sub(w, t);
  viper_decode(m, w);
}
